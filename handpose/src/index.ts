...
async function loadHandDetectorModel() {
  const HANDDETECT_MODEL_PATH =
      'https://hub.tensorflow.google.cn/mediapipe/tfjs-model/handdetector/1/default/1';
...
async function loadHandPoseModel() {
  const HANDPOSE_MODEL_PATH =
      'https://hub.tensorflow.google.cn/mediapipe/tfjs-model/handskeleton/1/default/1';
...
async function loadAnchors() {
  return tf.util
      .fetch(
          'https://cdn.aimaker.space/tfhub-tfjs-modules/mediapipe/tfjs-model/handskeleton/1/default/1/anchors.json')
      .then(d => d.json());
}
